...
public class EvilMage : Enemy
{
    ...
    {
        GameObject obj = Instantiate(damageZone);
        obj.transform.position = new Vector3(transform.position.x, 1, transform.position.z);
        Vector3 vvv = (player.transform.position - transform.position).normalized * objSpeed;
        vvv.y = 0;
        obj.GetComponent<Rigidbody>().velocity = vvv;
    }
}
